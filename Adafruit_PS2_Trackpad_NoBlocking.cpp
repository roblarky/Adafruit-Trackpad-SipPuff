...
void Adafruit_PS2::constructor(uint8_t c, uint8_t d) {
  _clk = c;
  _data = d;
  _readingTouchpadData = false;
  _disableTouchpadRead = false;
}
...
boolean Adafruit_PS2_Trackpad::readData(uint8_t clockWaits) {
  //write(0x0F);
  //Serial.println("start readData");
  if (_disableTouchpadRead) return false;
  if (clockWaits > 255) clockWaits = 254;
  _readingTouchpadData = false;
  uint8_t packet[6];
  for (uint8_t x=0; x<6; x++) {
    //Max 254 - how many clock high while loops before returning
    //To support the loop() being called repeatedly for other code
    packet[x] = read(clockWaits);
    if (x == 0 && packet[x] == 0x00)
    {
        return false;
    }
...
uint8_t Adafruit_PS2::read(uint8_t clockWaits) //was (void)
{
  uint16_t d = 0;
  ...
  for (uint8_t i=0; i<11; i++) {
    ...
    {
        if (!_readingTouchpadData && clockWaits != 0)
        {
            clockCount++;
            //Clock count loop exceeded, set inhibit and return
            //to continue processing other code in loop()
            if (clockCount > clockWaits)
            {
                //Not sure if we need to drain bits, see linux ps2_drain method
                inhibit();
                return 0x00;
            }
        }
    }
    _readingTouchpadData = true;
